...
"""
╔══════════════════════════════════════════════════════════════════════════════════╗
║        QMC FRAMEWORK v2.7.3 - VALIDATION EXHAUSTIVE COMPLÈTE                     ║
...
║  ├── [SECTION 5d] ★★ v2.7.3: MULTI-JOB SESSION MANAGER ★★                      ║
...
║  ├── [SECTION 5e] ★★ v2.7.3: PARALLEL TRANSPILATION + PROGRESS BAR ★★          ║
...
╠══════════════════════════════════════════════════════════════════════════════════╣
║  Usage:                                                                          ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py                                     ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py --backend ibm_fez --shots 100       ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py --skip-qpu      # Tests locaux      ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py --only-multijob # Multi-job seul    ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py --multijob-qpu  # Multi-job QPU     ║
║    python test_qmc_v2_7_3_FULL_VALIDATION.py --multijob-sim  # Multi-job SIM     ║
╚══════════════════════════════════════════════════════════════════════════════════╝
"""
...
# ══════════════════════════════════════════════════════════════════════════════
# [v2.7.3] ADAPTATION — rediriger les imports du test vers le module v2.7.3.
# On charge qmc_quantum_framework_v2_7_3.py et on l'enregistre dans sys.modules
# SOUS LES DEUX NOMS, de sorte que tous les `importlib.import_module('..._v2_7_3')`
# et `from ..._v2_7_3 import ...` du test résolvent la v2.7.3 sans éditer chaque ligne.
...
_v271_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                          "qmc_quantum_framework_v2_7_3.py")
_spec_271 = _ilu.spec_from_file_location("qmc_quantum_framework_v2_7_3", _v271_path)
_mod_271 = _ilu.module_from_spec(_spec_271)
sys.modules["qmc_quantum_framework_v2_7_3"] = _mod_271
_spec_271.loader.exec_module(_mod_271)
print(f"[v2.7.3 TEST] Module chargé: v{getattr(_mod_271, '__version__', '?')} "
...
def print_banner():
    print(f"""
{Colors.CYAN}╔══════════════════════════════════════════════════════════════════════════════════╗
║{Colors.BOLD}      ⚛️  QMC FRAMEWORK v2.7.3 - VALIDATION EXHAUSTIVE COMPLÈTE  ⚛️            {Colors.END}{Colors.CYAN}║
╠══════════════════════════════════════════════════════════════════════════════════╣
║  Test de TOUS les composants: 21 Builders, 12 Analyzers, Optimizers, etc.        ║
║  ★ v2.7.3 MAJOR: Multi-Job Session + Parallel Transpilation + Progress Bar      ║
...
def test_v271_multijob_session(fw, modules: Dict, mode: str = "local") -> bool:
    """
    Teste le Multi-Job Session Manager v2.7.3.
    ...
    """
    print_section("v2.7.3: MULTI-JOB SESSION MANAGER ★★", "🔄")
    ...
    try:
        # Import direct
        import importlib
        qmc_module = importlib.import_module('qmc_quantum_framework_v2_7_3')
        ...
        if hasattr(qmc_module, 'MultiJobSession'):
            ...
            results.add("MultiJobSession class", "PASS", category="v2.7.3")
            features_tested += 1
        else:
            print_test("MultiJobSession", "FAIL", "Non trouvée")
            results.add("MultiJobSession class", "FAIL", category="v2.7.3")
            all_passed = False
        ...
        if hasattr(qmc_module, 'MultiJobSessionStatus'):
            ...
            results.add("MultiJobSessionStatus class", "PASS", category="v2.7.3")
            features_tested += 1
        ...
        if hasattr(qmc_module, 'find_session_file'):
            ...
            results.add("find_session_file", "PASS", category="v2.7.3")
            features_tested += 1
        else:
        ...
        results.add("Import Multi-Job classes", "FAIL", str(e)[:50], category="v2.7.3")
        all_passed = False
    ...
    try:
        ...
        if hasattr(fw, 'run_multi_job_session'):
            ...
            results.add("fw.run_multi_job_session()", "PASS", category="v2.7.3")
            features_tested += 1
        ...
        if hasattr(fw, 'print_sessions_list'):
            ...
            results.add("fw.print_sessions_list()", "PASS", category="v2.7.3")
            features_tested += 1
        else:
    ...
    if MultiJobSession and MultiJobSessionStatus:
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                results.add("MultiJobSession manipulation", "PASS", category="v2.7.3")
                
            except Exception as e:
                print_test("Manipulation session", "FAIL", str(e)[:50])
                results.add("MultiJobSession manipulation", "FAIL", str(e)[:50], category="v2.7.3")
                all_passed = False
                traceback.print_exc()
    else:
    ...
    # --- Résumé ---
    print(f"\n    {Colors.GREEN}✅ {features_tested} fonctionnalités Multi-Job v2.7.3 testées{Colors.END}")
    
    if all_passed:
        print(f"    {Colors.GREEN}✅ TOUS LES TESTS MULTI-JOB PASSÉS{Colors.END}")
        results.add("v2.7.3 Multi-Job Session", "PASS", f"{features_tested} features", category="v2.7.3")
    else:
        print(f"    {Colors.RED}❌ CERTAINS TESTS MULTI-JOB ONT ÉCHOUÉ{Colors.END}")
        results.add("v2.7.3 Multi-Job Session", "FAIL", category="v2.7.3")
...
def _test_multijob_simulation(fw, modules: Dict) -> bool:
    ...
    try:
        ...
        if summary["completed"] == len(jobs):
            ...
            results.add("Multi-Job Simulation", "PASS", f"{len(jobs)} jobs avec archives", category="v2.7.3")
            return True
        else:
            print_test("Multi-Job Simulation", "WARN", f"{summary['completed']}/{len(jobs)} jobs")
            results.add("Multi-Job Simulation", "WARN", f"{summary['completed']}/{len(jobs)}", category="v2.7.3")
        ...
        results.add("Multi-Job Simulation", "FAIL", str(e)[:50], category="v2.7.3")
        traceback.print_exc()
        return False
...
def _test_multijob_simulation_basic(fw, modules: Dict) -> bool:
    ...
    try:
        ...
        results.add("Multi-Job Simulation (basic)", "PASS", "4 jobs", category="v2.7.3")
        
        return True
        
    except ImportError:
        print_test("Simulation Basique", "SKIP", "qiskit-aer non installé")
        results.add("Multi-Job Simulation", "SKIP", "qiskit-aer manquant", category="v2.7.3")
        return True
...
def _test_multijob_qpu(fw, modules: Dict, shots: int = 200, archive_project: str = None) -> bool:
    ...
    try:
        ...
        if not fw._connected:
            ...
            results.add("Multi-Job QPU", "FAIL", "Non connecté", category="v2.7.3")
            return False
        ...
        try:
            ...
            if multijob_results is None:
                ...
                results.add("Multi-Job QPU", "FAIL", "Résultats nuls", category="v2.7.3")
                return False
            ...
            if status in ['COMPLETED', 'PARTIAL'] and completed > 0:
                ...
                results.add("Multi-Job QPU Execution", "PASS", f"{completed}/{total} jobs", category="v2.7.3")
                return True
            else:
                print_test("Test Multi-Job QPU", "FAIL", f"Status: {status}")
                results.add("Multi-Job QPU Execution", "FAIL", f"Status: {status}", category="v2.7.3")
                return False
                
        except KeyboardInterrupt:
            print_test("Multi-Job QPU", "WARN", "Interrompu par l'utilisateur")
            results.add("Multi-Job QPU", "SKIP", "Interrompu", category="v2.7.3")
            return True
            
    except Exception as e:
        print_test("Multi-Job QPU", "FAIL", str(e)[:50])
        results.add("Multi-Job QPU", "FAIL", str(e)[:50], category="v2.7.3")
        traceback.print_exc()
        return False
...
def test_v271_parallel_transpilation(fw, modules: Dict) -> bool:
    """
    Teste la transpilation parallèle v2.7.3.
    ...
    """
    print_section("v2.7.3: PARALLEL TRANSPILATION + PROGRESS BAR ★★", "⚡")
    ...
    try:
        ...
        if hasattr(qmc_module, '_parallel_transpile_worker'):
            ...
            results.add("_parallel_transpile_worker", "PASS", category="v2.7.3-transpile")
            features_tested += 1
            ...
            results.add("_parallel_transpile_worker", "FAIL", category="v2.7.3-transpile")
            all_passed = False
    ...
    try:
        if hasattr(fw, 'transpile_circuits'):
            ...
            if not missing_params:
                ...
                results.add("transpile_circuits() new params", "PASS", category="v2.7.3-transpile")
                features_tested += 1
                ...
                results.add("transpile_circuits() new params", "FAIL", category="v2.7.3-transpile")
                all_passed = False
        else:
    ...
    try:
        if hasattr(fw, 'transpile_parallel'):
            ...
            if 'transpile_circuits' in source and "parallel='always'" in source:
                ...
                results.add("transpile_parallel delegation", "PASS", category="v2.7.3-transpile")
                features_tested += 1
            else:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            try:
                ...
                if loaded.jobs[0].get("auto_transpile") == False:
                    ...
                    results.add("auto_transpile persistence", "PASS", category="v2.7.3-transpile")
                    features_tested += 1
                else:
                ...
                results.add("MultiJobSession auto_transpile", "FAIL", str(e)[:50], category="v2.7.3-transpile")
                all_passed = False
                traceback.print_exc()
    else:
    ...
    # --- Résumé ---
    print(f"\n    {Colors.GREEN}✅ {features_tested} fonctionnalités Parallel Transpilation v2.7.3 testées{Colors.END}")
    
    if all_passed:
        print(f"    {Colors.GREEN}✅ TOUS LES TESTS PARALLEL TRANSPILATION PASSÉS{Colors.END}")
        results.add("v2.7.3 Parallel Transpilation", "PASS", f"{features_tested} features", category="v2.7.3-transpile")
    else:
        print(f"    {Colors.RED}❌ CERTAINS TESTS ONT ÉCHOUÉ{Colors.END}")
        results.add("v2.7.3 Parallel Transpilation", "FAIL", category="v2.7.3-transpile")
    
    return all_passed


def test_v272_external_module_loader(fw, modules: Dict) -> bool:
    """[v2.7.3] Valide le chargement des modules EXTERNES (brevets) à la demande.
    ...
    all_passed = True
    try:
        import qmc_quantum_framework_v2_7_3 as qm
        ...
        if not gone:
            results.add("Aucune classe brevet exposée", "PASS", category="v2.7.3-modules")
            print_test("Classes brevet retirées du framework", "PASS")
        else:
            results.add("Aucune classe brevet exposée", "FAIL", ",".join(gone), category="v2.7.3-modules")
        ...
        if hasattr(qm, "QMCModule") and hasattr(qm, "QMCModuleNotAvailableError"):
            results.add("Interface QMCModule + erreur dédiée", "PASS", category="v2.7.3-modules")
            print_test("QMCModule + QMCModuleNotAvailableError présents", "PASS")
        else:
            results.add("Interface QMCModule + erreur dédiée", "FAIL", category="v2.7.3-modules")
        ...
        if res.get("success") and res.get("echo") == {"x": 7}:
            results.add("load_module(externe).run()", "PASS", "demo_echo", category="v2.7.3-modules")
            print_test("Chargement + exécution module externe", "PASS", "demo_echo")
        else:
            results.add("load_module(externe).run()", "FAIL", category="v2.7.3-modules")
        ...
        try:
            ...
            results.add("Erreur si module absent", "FAIL", "pas d'exception", category="v2.7.3-modules")
            print_test("QMCModuleNotAvailableError si module absent", "FAIL")
            all_passed = False
        except qm.QMCModuleNotAvailableError:
            results.add("Erreur si module absent", "PASS", category="v2.7.3-modules")
        ...
        results.add("v2.7.3 External Module Loader", "FAIL", str(e)[:50], category="v2.7.3-modules")
...
def import_framework() -> Tuple[bool, Dict]:
    """Importe le framework v2.7.3 et ses composants."""
    ...
    try:
        from qmc_quantum_framework_v2_7_3 import (
            QMCFramework,
            MultiJobSession,
        ...
        print_test("Import QMCFramework", "PASS", "v2.7.3")
        ...
        results.add("Import Framework", "PASS", "v2.7.3", category="Import")
        
        # Optionnels
        try:
            from qmc_quantum_framework_v2_7_3 import RunMode
...
def main():
    parser = argparse.ArgumentParser(
        description='QMC Framework v2.7.3 - Validation EXHAUSTIVE',
...
Exemples:
  ...
  # Sans QPU
  python test_qmc_v2_7_3_FULL_VALIDATION.py --skip-qpu
  
  # Multi-Job seulement (tests locaux)
  python test_qmc_v2_7_3_FULL_VALIDATION.py --only-multijob
  
  # Multi-Job avec simulation
  python test_qmc_v2_7_3_FULL_VALIDATION.py --multijob-sim
  
  # Multi-Job sur QPU réel
  python test_qmc_v2_7_3_FULL_VALIDATION.py --multijob-qpu --backend ibm_fez
  
  # Multi-Job QPU avec upload vers projet
  python test_qmc_v2_7_3_FULL_VALIDATION.py --multijob-qpu --archive-project UUID
  
  # ★ REPRENDRE une session interrompue:
  python test_qmc_v2_7_3_FULL_VALIDATION.py --resume-session SESSION_20260129_110752_abc123
"""
    )
    ...
    try:
        ...
        print_test("Instance QMCFramework", "PASS", "v2.7.3")
    ...
    if args.resume_session:
        ...
        try:
            ...
            if resume_results:
                ...
                results.add("Resume Session", "PASS", f"{completed}/{total} jobs", category="v2.7.3")
            else:
                print_test("Reprise session", "FAIL", "Résultats nuls")
                results.add("Resume Session", "FAIL", category="v2.7.3")
                
        except Exception as e:
            print_test("Reprise session", "FAIL", str(e)[:50])
            results.add("Resume Session", "FAIL", str(e)[:50], category="v2.7.3")
            traceback.print_exc()
    ...
    # Tests v2.7.3 Multi-Job (toujours exécutés en mode complet)
    multijob_mode = "local" if args.skip_qpu else "all"
    test_v271_multijob_session(fw, modules, mode=multijob_mode)
    
    # Tests v2.7.3 Parallel Transpilation (toujours exécutés)
    test_v271_parallel_transpilation(fw, modules)

    # [v2.7.3] Chargeur de modules externes (brevets fournis séparément)
    ...
    print(f"    {Colors.YELLOW}   Les fonctionnalités v2.6.x sont rétro-compatibles avec v2.7.3{Colors.END}")
    ...
    if success:
        ...
        print(f"  ✅ VALIDATION v2.7.3 RÉUSSIE (Multi-Job + Parallel Transpilation)")
        print(f"{'═' * 80}{Colors.END}")
    else:
        print(f"{Colors.RED}{'═' * 80}")
        print(f"  ❌ VALIDATION v2.7.3 ÉCHOUÉE")
        print(f"{'═' * 80}{Colors.END}")
    print()
